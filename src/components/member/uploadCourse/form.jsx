...
export default function Form({
	...
	const [uploadCourseAlertDisplay, SetUploadCourseAlertDisplay] =
		useState(false);
		...
		if (answer) {
			const handleData = async () => {
				const response = await fetch(
					`/api/teacher/putCourse?courseSN=${answer}`
				)
					.then(r => r.json())
					.catch(err => err);
	...
	const [courseDescriptionLimiteMetion, setCourseDescriptionLimiteMetion] =
		useState('white');
	const [instructorExperienceLimite, setInstructorExperienceLimite] =
		useState('');
	const [
		instructorExperienceLimiteMetion,
		setInstructorExperienceLimiteMetion
	] = useState('white');
	...
	const [selectVideoFileLimiteMetion, setSelectVideoFileLimiteMetion] =
		useState('white');
	...
	const handleSumbit = async e => {
		...
		if (selectFile === null) {
			...
			ext.some(v => {
				return v === extname(selectFile.name).substring(1);
			})
		) {
			console.log('符合');
			setSelectFileLimite('');
		...
		if (courseSN) {
			...
			try {
				await axios
					.post('/api/teacher/putCourse', formdata, {
						headers: {
					...
					.then(response => console.log('上傳成功'));
				setDone(true);
			...
			try {
				const response = await axios
					.post('/api/teacher/uploadCourse', formdata, {
						headers: {
							'Content-Type': 'multipart/form-data' // 設置標頭，表明傳送的是 FormData
						},
						onUploadProgress: progressEvent => {
		...
		<>
			<form
						...
						<div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
							...
							<div className="col-span-full">
								...
								<div className="mt-2">
									<Quill
										...
										metion={courseDescriptionLimite}
									/>
								</div>
								<p className="mt-1 text-xs leading-6 text-gray-400">
									輸入說明至少達200字元以上
								...
								<p
									className={`text-xs leading-6 text-${courseDescriptionLimiteMetion}`}>
									內容未達200字元
								</p>
							</div>
							...
							<div className="col-span-full">
								...
								<div className="mt-2  shadow-sm">
									<Quill
										setText={setInstructorExperience}
										text={instructorExperience}
										metion={instructorExperienceLimite}
									/>
								</div>
								<p className="mt-1 text-xs leading-6 text-gray-400">
									輸入說明至少達200字元以上
								...
								<p
									className={`text-xs leading-6 text-${instructorExperienceLimiteMetion} `}>
									內容未達200字元
								</p>
							</div>
									...
									<div className="sm:flex ">
										...
										<div className="sm:w-6/12">
											<p className="text-base text-justify">
												在此上傳您的課程圖片。必須符合我們的課程圖片品質標準方可使用。重要規範：1920x1080
												像素；jpg、jpeg、png 檔案類型，圖片上不可有文字。
											</p>
											<div className="mt-5">
												<label
													htmlFor="file-input"
												...
												/>
												<p
													className={`text-xs leading-6 text-${selectFileLimiteMetion} `}>
													{selectFile === null
														? '此欄位必填'
														: '請上傳jpg、jpeg、png檔'}
												</p>
							...
							<div className=" col-span-full">
								...
								<div className="mt-2">
									<div className="sm:flex ">
										...
										<div className="sm:w-6/12">
											<p className="text-base text-justify">
												製作一部引人入勝的宣傳影片，能夠吸引學生的注意力，並迅速介紹您課程的內容，讓他們對所學內容有更清晰的預覽。若影片製作精良，學生報名您課程的可能性將大大提高。重要規範：請上傳
												mp4 檔案類型。
											</p>
											<div className="mt-5">
												<label
												...
												/>
												<p
													className={`text-xs leading-6 text-${selectVideoFileLimiteMetion} `}>
							...
							<div className="col-span-full">
								...
								<div className="mt-2">
									<div className="sm:flex overflow-y-scroll">
										<UploadTables
											UploadFileAlertDisplay={UploadFileAlertDisplay}
											SetUploadFileAlertDisplay={SetUploadFileAlertDisplay}
											{...{ lessonData }}
											putData={putData}
				...
				<div className="mt-4 flex items-center justify-center  md:justify-end gap-x-6">
					<button
						type="button"
						className="ring-1 ring-orange-500 px-3 py-2 text-sm font-semibold text-orange-500 shadow-sm hover:ring-orange-300 hover:text-orange-300"
						onClick={() => {
			...
			<UploadCourseAlert
				progress={35}
				uploadCourseAlertDisplay={uploadCourseAlertDisplay}
				progressEl={progressEl}
				SetUploadCourseAlertDisplay={SetUploadCourseAlertDisplay}
				done={done}
			/>
		</>
	);
