...
export default function UploadFileAlert({
		...
		if (putData) {
			...
			try {
				...
				const response = await axios
					.post('/api/teacher/uploadVideo', formData, {
						headers: {
	...
	return (
		<div
			className={`bg-white flex flex-col items-center shadow-sm ring-1 ring-gray-300 fixed w-full h-full top-0 left-0 md:w-3/5 md:top-1/2 md:left-1/2 md:transform md:-translate-x-1/2 justify-center md:-translate-y-1/2 px-5 overflow-scroll md:h-4/6	${UploadFileAlertDisplay}
		`}>
			...
			<div className="block text-md font-semibold leading-6 text-gray-900 mt-5">
				請填入上傳所需的資料
			</div>

			{/* 表單開關 */}
			<form
				className={`flex w-full flex-col items-center my-5 ${displayForm}`}
				...
				<div className="mt-2">
					<button
						type="button"
						className="ring-1 ring-orange-500 text-orange-500 px-6 py-2 text-sm font-semibold  shadow-sm hover:text-orange-300  hover:ring-orange-300"
						onClick={e => {
							e.preventDefault();
				...
				{!!video && (
					<div className="text-center">
						...
						<progress
							...
							value={0}
							max={100}
							className="flex border h-3 rounded-full sm:max-w-md w-full  mt-5"
						/>
					</div>
				)}
				{done || (
					<div className="my-5">請不要未完成檔案上傳就關閉頁面，避免出錯</div>
				)}
				{done && (
					<div className="flex flex-col items-center my-5">
						...
						<button
							type="submit"
							className=" bg-orange-400 px-3 py-2 text-sm font-semibold text-white shadow-sm mt-10 hover:bg-orange-300"
							onClick={() => {
								SetUploadFileAlertDisplay('hidden');
								setDisplayForm('');
								setVideo(null);
