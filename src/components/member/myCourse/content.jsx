...
export default function Content({ domain, search }) {
	...
	const handleData = async () => {
		try {
			const response = await fetch(
				`/api/learner/myCourse/filter${domain ? `?domain=${domain}` : ''}${
	...
	return (
		<div className={styles.content}>
			<div
				className={`grid grid-cols-1 w-full md:grid-cols-3 gap-x-4 gap-y-4 md:gap-x-8 md:gap-y-8 md:gap-y-16 mt-10 sm:mt-16`}>
				{data.length === 0 && (
					<div
